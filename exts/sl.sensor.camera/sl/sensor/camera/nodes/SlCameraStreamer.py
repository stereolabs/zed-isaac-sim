...
ZED_X_VIRTUAL_CAMERA_IDS = [40976320, 41116066, 49123828, 45626933]
ZED_X_MINI_VIRTUAL_CAMERA_IDS = [57890353, 55263213, 57800035, 57706147]
...
class SlCameraStreamer:
    ...
    @staticmethod
    def check_camera(camera_prim_path : str, resolution: list[int, int]):
        ...
        if is_prim_path_valid(camera_prim_path) == True:
            ...
            # Pixel size in microns, aperture and from the camera sensor specification
            pixel_size = 3 * 1e-3   # in mm, 3 microns is a common pixel size for high resolution cameras

            # Calculate the horizontal and vertical aperture size from the resolution and pixel size
            horizontal_aperture =  pixel_size * resolution[0] # The aperture size in mm
            vertical_aperture =  pixel_size * resolution[1]

            focal_length = 2.2 # Focal length in mm
            sdf_prim_path = Sdf.Path(camera_prim_path)

            omni.kit.commands.execute("ChangeProperty",
                                      prop_path=Sdf.Path(
                                          sdf_prim_path.AppendProperty("horizontalAperture")),
                                      value=horizontal_aperture,
                                      prev=fl.Get()
                                      )
            omni.kit.commands.execute("ChangeProperty",
                                      prop_path=Sdf.Path(
                                          sdf_prim_path.AppendProperty("verticalAperture")),
                                      value=vertical_aperture,
                                      prev=fl.Get()
                                      )
            omni.kit.commands.execute("ChangeProperty",
                                      prop_path=Sdf.Path(
                                          sdf_prim_path.AppendProperty("focalLength")),
                                      value=focal_length,
                                      prev=fl.Get()
                                      )
    ...
    def createStreamer(db) -> bool:
        db.per_instance_state.imu_sensor_interface = _sensor.acquire_imu_sensor_interface()
        db.per_instance_state.core_nodes_interface = _isaacsim_core_nodes.acquire_interface()
        ...
        db.per_instance_state.override_simulation_time = db.inputs.use_system_time
        if db.per_instance_state.override_simulation_time:
        ...
        if not len(db.inputs.camera_prim) == 1:
            ...
            return False
        db.per_instance_state.camera_prim_name = db.inputs.camera_prim[0].name
        ...
        camera_model = db.inputs.camera_model

        LEFT_CAMERA_PATH = "/base_link/" + camera_model + "/CameraLeft"
        RIGHT_CAMERA_PATH = "/base_link/" + camera_model + "/CameraRight"
        IMU_PRIM_PATH = "/base_link/" + camera_model + "/Imu_Sensor"
        ...
        # Check frame rate
        db.per_instance_state.target_fps = SlCameraStreamer.check_frame_rate(db.inputs.fps)

        if (db.per_instance_state.annotator_left is None):
            db.per_instance_state.render_product_path_left = SlCameraStreamer.get_render_product_path(left_cam_path, render_product_size=resolution)
            db.per_instance_state.annotator_left = rep.AnnotatorRegistry.get_annotator("rgb", device="cuda", do_array_copy=False)
            db.per_instance_state.annotator_left.attach([db.per_instance_state.render_product_path_left])

        if (db.per_instance_state.annotator_right is None):      
            db.per_instance_state.render_product_path_right = SlCameraStreamer.get_render_product_path(right_cam_path, render_product_size=resolution)     
            db.per_instance_state.annotator_right = rep.AnnotatorRegistry.get_annotator("rgb", device="cuda", do_array_copy=False)        
            db.per_instance_state.annotator_right.attach([db.per_instance_state.render_product_path_right])

        db.per_instance_state.imu_prim_path = db.inputs.camera_prim[0].pathString + IMU_PRIM_PATH
        db.per_instance_state.imu_prim = SingleXFormPrim(prim_path=db.per_instance_state.imu_prim_path)
        
        # Setup streamer parameters
        db.per_instance_state.pyzed_streamer = ZEDSimStreamer()

        # Check serial number
        serial_number = db.inputs.serial_number
        camera_ids = db.per_instance_state.pyzed_streamer.getVirtualCameraIdentifiers()
        ...
        # Check model according to serial number
        camera_model = db.inputs.camera_model
        if camera_model == "ZED_X":
            if serial_number not in ZED_X_VIRTUAL_CAMERA_IDS:
                carb.log_error(f"Invalid serial number passed. Please make sure the serial number is in the list: {ZED_X_VIRTUAL_CAMERA_IDS}")
                return False
        elif camera_model == "ZED_X_Mini":
            if serial_number not in ZED_X_MINI_VIRTUAL_CAMERA_IDS:
                carb.log_error(f"Invalid serial number passed. Please make sure the serial number is in the list: {ZED_X_MINI_VIRTUAL_CAMERA_IDS}")
                return False
        else:
            carb.log_error("Invalid camera model passed. Defaulting to ZED_X.")
            camera_model = "ZED_X"
            serial_number = ZED_X_VIRTUAL_CAMERA_IDS[0]

        db.per_instance_state.camera_model = camera_model

        streamer_params = ZEDSimStreamerParams()
        streamer_params.image_width = resolution[0]
        streamer_params.image_height = resolution[1]
        streamer_params.fps = db.per_instance_state.target_fps
        ...
        init_error = db.per_instance_state.pyzed_streamer.init(streamer_params)
        if not init_error:
            carb.log_error(f"Failed to initialize the ZED SDK streamer with serial number {serial_number}.")
            return False

        # set state to initialized
        carb.log_info(f"Streaming camera {db.per_instance_state.camera_prim_name} at port {port} and using serial number {serial_number}.")
        db.per_instance_state.invalid_images_count = 0
        db.per_instance_state.last_timestamp = 0.0
        db.per_instance_state.data_shape = (resolution[1], resolution[0], CHANNELS)
    ...
    def compute(db) -> bool:
        ...
        try:
            ts : int = 0
            if db.per_instance_state.initialized is True:
                left, right = None, None

                # Get simulation time in seconds
                current_time = db.per_instance_state.core_nodes_interface.get_sim_time()

                # Reset last_timestamp between different play sessions
                if db.per_instance_state.last_timestamp > current_time:
                    db.per_instance_state.last_timestamp = current_time

                delta_time = current_time - db.per_instance_state.last_timestamp

                # Skip data fetch if the time between frames is too short
                if delta_time < 1.0 / db.per_instance_state.target_fps:
                    return False
                
                # we fetch image data from annotators - we do it sequentially to avoid fetching both if one fails
                left, res = SlCameraStreamer.get_image_data(db.per_instance_state.annotator_left,
                                                            db.per_instance_state.data_shape)
                if res == False:
                    db.per_instance_state.invalid_images_count +=1
                    if db.per_instance_state.invalid_images_count >= 10:
                        carb.log_warn(f"{db.per_instance_state.camera_prim_name} - Left camera retrieved unexpected "
                                      "data shape, skipping frame.")
                    return False # no need to continue compute

                right, res = SlCameraStreamer.get_image_data(db.per_instance_state.annotator_right,
                                                             db.per_instance_state.data_shape)
                ...
                if res == False:
                    db.per_instance_state.invalid_images_count +=1
                    if db.per_instance_state.invalid_images_count >= 10:
                        carb.log_warn(f"{db.per_instance_state.camera_prim_name} - Right camera retrieved unexpected "
                                      "data shape, skipping frame.")
                    return False # no need to continue compute
                
                # Fetch simulation time and change its reference point to (system time at start)
                if db.per_instance_state.start_time == -1:
                    carb.log_info(f"{db.per_instance_state.camera_prim_name} - Starting stream to the ZED SDK")
                if db.per_instance_state.start_time == -1 or current_time < db.per_instance_state.last_timestamp:
                    db.per_instance_state.start_time = int(time.time_ns())
                    carb.log_info(f"{db.per_instance_state.camera_prim_name} - Setting initial streaming time stamp to: {db.per_instance_state.start_time}")

                ts = int(db.per_instance_state.start_time + current_time * 1000000000)
                ...
                orientation = [0]*4
                if is_prim_path_valid(db.per_instance_state.imu_prim_path) == True and db.per_instance_state.imu_prim is not None:
                    imu_prim = db.per_instance_state.imu_prim
                    ...
                    if(orientation[0] == 0 and orientation[1] == 0 and  orientation[2] == 0 and orientation[3] == 0):
                        carb.log_warn(f"{db.per_instance_state.camera_prim_name} - Received invalid orientation, skipped frame !")
                        return False
                    if imu_prim.is_valid() == True:
                        if db.per_instance_state.imu_sensor_interface.is_imu_sensor(db.per_instance_state.imu_prim_path) == True:
                            imu_reading = db.per_instance_state.imu_sensor_interface.get_sensor_reading(db.per_instance_state.imu_prim_path)
                ...
                if not (left is None) and not (right is None):
                    ...
                    res = db.per_instance_state.pyzed_streamer.stream(
                        left_bytes,
                        right_bytes,
                        ts,
                        orientation[0],
                    ...
                    if not res == 0:
                        if db.per_instance_state.invalid_images_count >= 10:
                            carb.log_warn(f"{db.per_instance_state.camera_prim_name} - Streaming failed at timestamp {ts} with error code: {res}")
                        db.per_instance_state.invalid_images_count += 1
                    else:
