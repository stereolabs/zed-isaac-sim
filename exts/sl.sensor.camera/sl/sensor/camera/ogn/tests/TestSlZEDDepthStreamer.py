...
class TestOgn(ogts.OmniGraphTestCase):
    ...
    async def test_data_access(self):
        from sl.sensor.camera.ogn.SlZEDDepthStreamerDatabase import SlZEDDepthStreamerDatabase
        test_file_name = "SlZEDDepthStreamerTemplate.usda"
        usd_path = os.path.join(os.path.dirname(__file__), "usd", test_file_name)
        if not os.path.exists(usd_path):  # pragma: no cover
            self.assertTrue(False, f"{usd_path} not found for loading test")
        (result, error) = await ogts.load_test_file(usd_path)
        self.assertTrue(result, f'{error} on {usd_path}')
        test_node = og.Controller.node("/TestGraph/Template_sl_sensor_camera_ZED_Depth")
        database = SlZEDDepthStreamerDatabase(test_node)
        ...
        self.assertTrue(test_node.get_attribute_exists("inputs:enableSave"))
        attribute = test_node.get_attribute("inputs:enableSave")
        self.assertTrue(attribute.is_valid())
        db_value = database.inputs.enableSave
        database.inputs.enableSave = db_value
        ...
        self.assertTrue(test_node.get_attribute_exists("inputs:outputDir"))
        attribute = test_node.get_attribute("inputs:outputDir")
        self.assertTrue(attribute.is_valid())
        db_value = database.inputs.outputDir
        database.inputs.outputDir = db_value
        expected_value = ""
        actual_value = og.Controller.get(attribute)
        ogts.verify_values(expected_value, actual_value, _attr_error(attribute, True))
        ogts.verify_values(expected_value, db_value, _attr_error(attribute, False))
